...
function getSavedConversationId(): number | null {
  ...
  const saved = sessionStorage.getItem('ticket_conversation_id');
...
export default function TicketAgentPage() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem('ticket_conversation_id')) setLoadingHistory(true);
  }, []);
  ...
  useEffect(() => {
    fetch(`${API}/api/agents/ticket_agent/config`).then(r => r.json()).then(d => {
      if (d.config) {
        setConfig({
  ...
  async function handleSaveConfig() {
    ...
    try {
      const r = await fetch(`${API}/api/agents/ticket_agent/config`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(config) });
      setSaveMsg(r.ok?'保存成功':'保存失败');
    } catch { setSaveMsg('保存失败'); }
    finally { setSaving(false); }
  }

  function handleNewChat() { setMessages([]); setConversationId(null); setTrace(null); sessionStorage.removeItem('ticket_conversation_id'); }
  ...
  async function handleSend() {
    ...
    try {
      ...
      const r = await fetch(`${API}/api/agents/ticket_agent/chat`, { method:'POST', headers:{'Content-Type':'application/json','X-Tenant-ID':String(TENANT_ID)}, body:JSON.stringify(body) });
      const data = await r.json();
      if (data.conversation_id && !conversationId) { setConversationId(data.conversation_id); sessionStorage.setItem('ticket_conversation_id', String(data.conversation_id)); }
    ...
    <div style={{ display:'flex', height:'100vh', background:T.bg, color:T.text, fontFamily:"system-ui,-apple-system,'Segoe UI',sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ padding:`${S.lg}px ${S.xl}px ${S.base}px` }}>
          <h3 style={{ margin:0, fontSize:16, fontWeight:600, color:T.text }}>工单智能体</h3>
          <p style={{ margin:0, marginTop:S.xs, fontSize:13, color:T.secondary }}>投诉处理、维修工单、售后查询</p>
        </div>
        <div style={{ flex:1, padding:`0 ${S.xl}px`, display:'flex', flexDirection:'column', gap:S.md, overflow:'auto', paddingBottom:S.xl }}>
          ...
          <div style={{ paddingTop:S.sm }}>
            <McpToolBinding agentKey="ticket_agent" />
          </div>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', background:T.bg }}>
        ...
        <div style={{ flex:1, padding:S.xl, overflow:'auto' }}>
          ...
          {!loadingHistory && messages.length===0 && <div style={{ textAlign:'center', color:T.secondary, marginTop:S.huge, fontSize:14 }}>输入客户售后问题，测试工单智能体</div>}
          {messages.map((msg, i) => {
            const isUser = msg.role === 'user';
            return (
              <div key={i} style={{ marginBottom:S.lg, display:'flex', flexDirection:'column', alignItems:isUser?'flex-end':'flex-start' }}>
                <div style={{ fontSize:12, color:T.secondary, marginBottom:6 }}>{isUser?'测试用户':'TicketAgent'} · {msg.time}</div>
